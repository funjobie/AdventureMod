...
public class AdventureStarterBlock extends BlockContainer
{
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int metadata, float what, float these, float are)
	{
		...
		player.openGui(AdventureMod.instance, ModGUIs.AdventureStarterGUIID, world, x, y, z);
		return true;
	}
	...
	private void dropItems(World world, int x, int y, int z)
	{
		...
		for (int i = 0; i < inventory.getSizeInventory(); i++)
		{
			...
			if (item != null && item.stackSize > 0)
			{
				...
				EntityItem entityItem = new EntityItem(world, x + rx, y + ry, z + rz, new ItemStack(Block.getBlockFromItem(item.getItem()), item.stackSize,
						item.getItemDamage()));
